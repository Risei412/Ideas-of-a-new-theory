...
2. **GKSL 物理実現（stop condition 2 の判定・現在の新規性の重心）:** custom Liouville 実装（QuTiP 不可）で
   ...
   **EIT §9.2（Λ系の `c = iΩ_p d₁/2`, `p† = d₁†`, `γ_g = γ₁₂ − iδ₂`）**。
   ...
   > **【2026-07-26】この Step の実行計画を `docs/dark-ridge-calculation-plan.md` に分離した。**
   > 三準位 Λ 系での weak-probe 感受率について、吸収側・分散側それぞれの回復曲線
   > `F_disp = Re(χ−χ_∞) = 0`、`F_abs = Im(χ−χ_∞) = 0` の存否を判定する工程表である。
   > 同計画 §24.1 は、凍結EIT §9.6 の閉形式（`ρ⁰=|g₁⟩⟨g₁|` 前提）からの手計算予想として
   > **吸収側は解なし・分散側のみ正の解あり（クラス D only）** を与えている。
   > ただしこの前提は真の定常状態では成り立たず、optical pumping による population inversion が
   > 正値性を破りうるため、**完全な Liouvillian 解で判定するまで結論としない。**
   > **【判定完了 2026-07-26】`docs/dark-ridge-novelty-verdict.md`。三準位 Λ 系では FREEZE。**
   > `scripts/dark_ridge_lambda_model.py`（exact、証明書
   > `certificates/dark_ridge_three_level_2026-07-26.txt`）で厳密に示された結果:
   > - `Ω_p=0` の定常状態は**厳密に `|g₁⟩⟨g₁|`**（`g₁` が sink）。population inversion は起きず、
   >   分岐比は `ρ⁰` に影響しない。→ 凍結EIT §9.6 の閉形式は近似ではなく厳密。
   > - `Im χ` の分子 `γ_31(γ_12²+δ²)+βγ_12` は**全単項式係数が正** ⇒ `γ_12>0` で厳密に正。
   >   **吸収回復曲線は存在しない。** すなわち**この GKSL クラスでは符号が強制される。**
   >   Y7 の「CPTP は符号を強制しない」は線形代数レベルの主張であり、
   >   上の留保「Step 2 で実際に導出するまで no-go 分岐を捨ててはならない」が正しかった。
   > - 分散側の稜線は存在するが `β = Δ_p(γ_12²+δ²)/δ` で、`γ_12→0` で ac-Stark 零点
   >   `β = Δ_p δ` へ還元される。⇒ 分散側は既知現象。
   >
   > **⇒ 三準位 Λ 系での稜線対分裂は死んだ（クラス D only、`Δ_split` 定義不能）。**
   >
   > **【Gate P1 追記 2026-07-26 同日】PRL（no-go）分岐も反例で死亡。**
   > `scripts/dark_ridge_pump_probe.py`（証明書 `certificates/dark_ridge_nogo_2026-07-26.txt`）。
   > `g₁` の sink 性だけを壊す jump を**1本だけ**追加すると、
   > **population inversion を伴わないまま `Im χ` が符号を変え、利得へ抜ける**
   > （`Ω_c=2, w=1/10` で `γ_12<0.07` 程度、`ρ_ee−ρ_{g₁g₁}≈−0.55`）。
   > `rate→0` では三準位の結果を厳密に回収する。
   > ⇒ **`Im χ` の符号を固定していたのは dark-state 構造ではなく `ρ⁰` の population 配置であり、
   > 「CPTP／dark state が符号を強制する」形の no-go は成立しない。**
   > しかも反例は**反転なし利得（LWI）**という確立分野に入るだけで、新規性にならない。
   >
   > **⇒ stop condition 2 は「実現可能（PRA）」でも「no-go（PRL）」でもなく、
   > 「実現可能だが既知現象」という第三の結末で決着した。物理 pullback 路線は FREEZE。**
   > 却下記録はガイド §6.4 と `docs/context-pack.md` §5 に転記済み。
   > **番号24は消費しない** — 抽象応答族での C1/C2（2-jet 恒等式・稜線二分律）は
   > 依然として exact であり、死んだのは物理 pullback である。
   > ただし §6 が「監査後に唯一残った新規性の重心」と述べていたのはこの pullback であるため、
   > **本提案の新規性の重心は現在いずれの候補も埋まっていない。**
...
- **最も不足している物理接続:** 全結果が線形代数レベル。GKSL/CPTP 実現（§5.2-2）と
  ...
  **【2026-07-26】この項目に対する実行計画を `docs/dark-ridge-calculation-plan.md` として起こした
  （新規提案番号は消費しない）。三準位 Λ 系での生死判定を Priority 0 とする。**
