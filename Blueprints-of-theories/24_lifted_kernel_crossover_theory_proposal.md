...
**判定:** **(b) 縮小継続。GKSL 実現可能性の判定は完了した（2026-07-26、§5.2 item 2）。**
結果は **次元二分律** — **`k = dim P = 1` は禁止**（定理 T1、すべての実線型汎関数について。
Λ/EIT はここに入り、支持条件でも外れるため二重に排除）、**`k ≥ 2` は実現可能**（定理 T2、構成的）。
**投稿先は PRA**（分類定理＋設計則「`dim P ≥ 2` が必要」＋実験提案）。
二段構えの PRL 分岐（CPTP no-go）は**発火しなかった**。
**PRX は現時点で不可**（§1.1 ゲート8項目中 Novelty が不合格）。
...
- **形（3つの予言）:**
  ...
  **理論固有なのは2つの切片とその差である。**
- **物理的翻訳:** `Γ` は光コヒーレンス damping スケール、`ε·Γ` は
  ground-state dephasing `γ₁₂`。稜線条件は **`Γ·γ₁₂ = Φ(α_leak)/Φ(κ_lift)`**。
  分散側と吸収側は**別々に測る量**であり、**両者が異なる `Γ·γ₁₂` で理想 dark-state 値へ
  回復する**ことが予言される。独立制御可能な2軸（励起状態崩壊と基底状態 dephasing）で走査できる。

  > ### ⚠️ 【2026-07-26 訂正】この予言を標準 EIT の透過／位相シフトで書いてはいけない
  >
  > 初版はこれを「透過（`Im χ`）と位相シフト（`Re χ`）」と書いていたが、
  > **GKSL 実現監査 Z8（`docs/lkct-gksl-realizability-audit.md` §3）で誤りと判明した。**
  >
  > 凍結 EIT §9.2 のブロックでは、光学コヒーレンス `x` が `Γ` スケールの damping
  > `A = γ₃₁ − iΔ_p` を担うため **F ブロック**、基底コヒーレンス `s`（`γ_g = γ₂₁ − iδ₂`）が
  > **P ブロック**である。ところが標準 EIT のプローブ配置では源が `(iΩ_p d₁/2, 0)ᵀ`、
  > 読み出しが `d₁†x` なので
  >
  > ```
  > c_P = 0,  c_F ≠ 0        p_P = 0,  p_F ≠ 0
  > ```
  >
  > となり、**LKCT が要求する `c=(c_P,0)`、`p=(p_P,0)` と正反対**。
  > **標準 EIT の透過・位相シフトはこの理論のクラスの外にある。**
  > さらに Λ 系は基底コヒーレンスが1本なので `k=1` であり、**定理 T1 によっても排除される**（二重排除）。
  >
  > **正しい実験翻訳:** 観測量は**基底コヒーレンスを直接読むもの**でなければならない
  > （Raman / RF / スピンコヒーレンス読み出し、二光子駆動源）。かつ **`k ≥ 2` が必要**なので、
  > 単一 Λ 系では足りず、**基底多様体に独立なコヒーレンスを2本以上持つ系**
  > （tripod、double-Λ 等）が要る。`Δ_split` の閉形式自体は生きているが、
...
2. ~~**GKSL 物理実現（stop condition 2 の判定・現在の新規性の重心）**~~ — **✅ 完了（2026-07-26）。
   判定 `docs/lkct-gksl-realizability-audit.md`、実装 `scripts/lkct_gksl_realize.py`（seed 20260729、
   Z0–Z9 全 PASS、判定に float 不使用）、証明書 `certificates/lkct_gksl_realizability_2026-07-26.txt`。**

   **結果は次元二分律。stop condition 2 は発火しない。**
   - **`k = dim P = 1` は禁止（定理 T1、Exact）。** Hermitian `H` と CPTP の下で
     `α_leak = −(m/d_L)·κ_lift` が厳密に成立し、比は**負の実数**で `p_P, c_P, B_PP` と
     全離調に依存しない。よって **Re/Im に限らずあらゆる実線型汎関数** `Φ` で
     `Φ(κ)Φ(α) = −(m/d_L)Φ(κ)² ≤ 0`、稜線座標 `Γ²ε = −m/d_L < 0` は物理域外。
   - **`k ≥ 2` は実現可能（定理 T2、Exact・構成的）。** 物理的 `B_PP = S+iT`
     （`S` 実対角 PSD、`T` 実対称）の下でも `Φ=Re` 19.7%・`Φ=Im` 18.5%。明示的厳密 witness を構成。
   - **起源:** 実 PSD 行列 `M = J D_F⁻¹ Jᵀ` と `(D_L)_PP` の**非比例性**。比例させると 0/2000 で
     T1 に厳密に退化する。
   - **補題 L2（Exact）:** `H = H†` より `B_PF = iJ_PF`、`B_FP = iJ_PFᵀ`（`J` 実）なので
     `C_leak = −J D_F⁻¹ Jᵀ` は**実の負半定値**。`arg C_leak = π` が厳密。
   - **補題 L1（Exact）:** `D₀ ⪰ 0` かつ `(D₀)_PP = 0` ⇒ `(D₀)_PF = 0`。
     Riesz 射影が自動的に直交射影になる（範囲宣言ではなく帰結）。
   - **負制御 NC1:** 非 Hermitian `H` にすると L2 が破れる。
     **`k=1` の no-go は Hermiticity が原因であることが確定**（「探したが無かった」ではない）。

   **⇒ 投稿先は PRA（分類定理＋設計則＋実験提案）。** no-go と実現可能性が次元で分かれるため、
   PRL 単独法則でも PRA 短報でもない。**PRX は依然不可**（Novelty ゲートの判断は不変）。

2bis. **GKSL pullback — ✅ 完了（2026-07-26 同日）。**
   ...
   **🆕 凍結理論の空白を2つ埋めた。**
   - **純位相緩和の縮約則** `L_φ = √γ_φ Σ_j a_j|j⟩⟨j|` ⇒ `ρ_{j,ref}` を `−½γ_φ(a_j−a_ref)²` で減衰
     （内訳: 再生項 `+γ_φ a_j a_ref`、反交換子 `−½γ_φ(a_j²+a_ref²)`）。
     EIT L.1115 が明示的に先送りし SMRT には "dephas" が皆無だった箇所であり、
     **LKCT の中心にある `(D_L)_PP` は本監査で初めて jump operator から導出された。**
     副産物として **`D_L` は対角に強制される**という構造的制約が判明。
   - **弱プローブ線形化そのもの。** SMRT `prop:phase-n` の証明は「linearization gives exactly
     the displayed pencil」と主張するのみで `½` すら計算していない（`rem:gksl-model` は
     各モデルでの導出を要求）。導出し、凍結の主張 `D = ½diag(d_j)`・`a = γ₁/2+iΔ₁`・
     `B_jk = iJ_jk` と一致することを確認した。

   **🆕 低速減衰は分裂の必要条件（V6）。** `γ_j → 0`（＝ `prop:phase-n` の `B_jj = 0`）にすると
   `B_PP` が純虚になり `κ, α` が整列して `Im(κ·conj α) = 0`、**分裂が消える**。
   `prop:phase-n` をそのまま使ってはいけない。

   **🆕 2つの稜線は漸近領域に入る `Γ` が桁で異なる**（吸収 ~2×10²、分散 ~2×10³）。
   両稜線の同時観測には `Γ ≳ 2×10³` が要る。新たな実験上の制約。

   **⚠️ 残る不足:** **物質系パラメータへの当てはめは未実施**（準位系は具体化されたが物質は未指定）。
   §1.2 降格条件「experimental pullback ができず抽象 parameter のまま」は
   **完全には解消されていない。**
