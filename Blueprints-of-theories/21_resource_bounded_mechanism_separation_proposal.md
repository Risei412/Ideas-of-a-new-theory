...
>
> (ii) **［full 側の容量超過］** 𝒲_full 上の generalized Hankel 行列 `H_full`（43×43）は
> `rank(H_full) ≥ 7` を満たし、これは非零 7×7 minor の exact arithmetic 評価で認証される。
> 同時に `H_cal = H_full[:7,:7]` は `rank(H_cal) ≤ 6`（(i) の帰結として自動成立）。
>
> (iii) **［有限 shot での認証可能性］** `σ₇(H_full) ≥ 2τ_H = 0.2933`（2000 shots/setting、settings ≤ 2、
...
- **数値的土台（実施済み）:** ステップ2スモーク（`scripts/sigma7_smoke.py`）の結果を前提とする — Hankel 43×43、H_cal 7×7、opnorm 正規化採用、要求水準 `σ₇ ≥ 2τ_H = 0.2933`、ランダム構成の合格率 6.8%（中央値 0.162）に対し射影付き山登りで 0.609 到達、判定 GO。
...
| `H_full`, `H_cal` | generalized Hankel（43×43）とその左上 7×7 ブロック | — |
...
**定義 1（観測インターフェース）.** alphabet サイズ m、最大 depth 4、prefix/suffix 長 ≤2 の語集合を固定する。データは `{R(w) : w ∈ 𝒲_full}`、各値は確率スケール `[−1,1]`（opnorm 正規化により `|R(w)| ≤ 1` 保証）。shot 予算は凍結値（calibration ≤ 1.2×10⁵ shots、held-out 独立予算 ≥ 1.28×10⁵、1000–2000 shots/setting、settings ≤ 2）。
...
(i) `P_{M_D}(w) = R_full(w)` ∀`w ∈ 𝒲_cal`（exact）、(ii) 非零 7×7 minor による `rank(H_full) ≥ 7`（exact）、(iii) `σ₇(H_full) ≥ 2τ_H`（数値、誤差評価付き）。
...
**補題 2（ノイズ頑健性）.** `‖ΔH‖₂ ≤ τ_H` なら Weyl の不等式より `σ₇(H_full + ΔH) ≥ σ₇(H_full) − τ_H > 0`。判定条件は `σ₇ > τ_H(α)`。status: Exact（Weyl）。`τ_H(α)` の統計模型はステップ5（未完）。
...
- **物理量:** 有限 shot 推定された Hankel 行列 `Ĥ_full` の第7特異値 `σ₇(Ĥ_full)`、および calibration 残差 `max_{w∈𝒲_cal} |P_{M_D}(w) − R̂(w)|`。
- **現れ方:** 閾値型。`σ₇(Ĥ_full) > τ_H(α)` かつ calibration 残差が二項誤差内、という**同時成立**が signature。単独ではどちらも自明。
- **検出手段とコスト:** 数値実験（custom NumPy/SymPy、QuTiP/CVXPY 不要）。shot コストは凍結予算内で完結する設計（m=6、1 setting: calibration 92,000 shots、depth-4 held-out 128,000 shots、合計 220,000）。実験実装は将来課題（§6）。
- **有限サイズ・有限精度:** ステップ2で確認済み — 理想データで σ₇=0.609 まで構成可能、要求 0.2933（1–2 settings）に対し余裕あり。有限 shot での可視性はまさに (iii) が保証する設計になっている。
...
4. **最適化:** 目的関数 `σ₇(H_full)`、制約（非負性・接木線形制約・opnorm）付き射影山登り。ステップ2の実績（120 反復で 0.402→0.609）と同じ機構をブロック構造上で回す。目標 `σ₇ ≥ 0.44`（= 1.5 × 0.2933、マージン付き）、下限合格 0.2933（1–2 settings）。
5. **有理化:** 最適化解を分母上限付き有理数へ丸め、接木制約を exact に再充足（線形制約なので有理射影で厳密に解ける）。丸めによる σ₇ 劣化を測り、劣化後も ≥ 2τ_H を確認。
...
1. `H_full` の 43×43 全成分を `sympy.Rational` の行列積で厳密計算（word ごとに `p† V_w c`）。prefix–suffix 閉包を保つ添字生成は `sigma7_smoke.py` の `words_upto` と同一規約（spec §1 の警告対応）。
...
4. **独立実装照合（§10.1）:** NumPy 倍精度の SVD で σ 系列を計算し、exact rank 判定と矛盾しないこと（σ₇ ≫ σ₈ ≈ 0…ではなく本件は σ₈ 以降が真に零か小さいか、を含めて記録）。
5. `σ₇` は浮動小数点で報告し、後退誤差上界（‖H‖ε_mach スケール）を併記。**exact 主張は rank のみ、σ₇ は数値量**と明記する（§9 禁止事項の遵守）。
...
- 例外集合からの距離に対するマージン安定性：構成点の近傍摂動（相対 10⁻³〜10⁻¹）で (i) の残差と σ₇ の変化を測る。認証は摂動後の再射影点で行う。
- shots/setting の格下げ表（2000/1304/652）に対する合否マップ。settings ≤ 2 の凍結制約下での動作点を明記。
...
| calibration | 46 protocols × 1–2 settings × 1000–2000 shots ≤ 1.2×10⁵ | OK（1 setting: 92,000） |
| held-out | depth-4 64 protocols、seed 20260725、≥ 1.28×10⁵（独立予算） | OK |
| settings/protocol | ≤ 2 | 凍結上限 |
| σ₇ 要求 | 0.2933（1 setting）／0.3632（2 settings） | ステップ2 GO 済み |
...
- **最も弱い仮定:** 接木 ansatz（`f_u† b_0 = 0`）が opnorm 制約・非負制約と同時に σ₇ ≥ 0.2933 を許すか。ステップ2のスモークは**ブロック構造なし**のランダム実現で行われており、構造制約付きで到達可能かは未検証。ここが折れたら中心命題ごと折れる。
- **最も脆い数値操作:** (a) 有理化での σ₇ 劣化、(b) minor 選択の pivot 依存（零 minor を引き続ける可能性）、(c) σ₇ と 2τ_H の差が小さい場合の判定の precision 依存。
...
- **stop condition:** (1) ステップ A の最適化が 10⁴ 反復オーダーで σ₇ ≥ 0.2933 に到達しない、(2) kill test D が成立、(3) ステップ C3 で非負性が空転と判明、(4) Stage 2 の 3 チャット判定で 1 回でも還元成功 — のいずれかで中心主張を撤回・縮小し §6.4 へ記録する。
- **その他未解決:** `τ_H(α)` 未導出（ステップ5）。ステップ2の分位点は 59–120 例で標本が小さい（spec の留意事項）— 設計凍結前に再実行。QHMM（族 DQ）の定義固定は本提案では扱わない。
