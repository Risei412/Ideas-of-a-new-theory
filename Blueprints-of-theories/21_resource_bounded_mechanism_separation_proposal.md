...
> **2026-07-26 現況:** 理想データに対する部分（明示構成と rank ≥ 7 の exact 認証）は**達成した**。
> **「有限 shot 予算内で認証できる」という部分が未達**であり、そこが本提案の現在の争点である。
...
**判定:** **要再設計**（2026-07-26）。中心命題の **(i)(ii) は構成的に証明された**（exact arithmetic、
`certificates/p0d_certificate_2026-07-26.txt`）が、**(iii) 有限shot余裕は凍結予算で未達**
（要求の 0.42–0.53倍、`p0-certificate-spec.md` §4quinquies）。
判定方式を作用素ノルム＋Weyl から統計的検定へ置き換えるか、予算を 3.6倍にするかの決着が要る。
到達できた場合の想定は **PRL 型**。P0-D（古典 HMM 族の排除）単独では P0 全体の達成ではない（`p0-certificate-spec.md` §5）。P0-A / P0-E / P0-DQ / P0-C は独立 gate であり、本提案の範囲外。複数族を排除できた時点で PRX 型へ昇格を検討する。
...
> **［認証付き実現ギャップ / Certified Realization Gap］**
> **status（2026-07-26 更新）: (i)(ii) は Exact（明示構成＋exact arithmetic で証明済み）／
> (iii) は未達（凍結予算では成立しない）**
>
...
>
> (iii) **［有限 shot での認証可能性］** `σ₇(H_wit) / （応答のレンジ幅） ≥ 2√8/√n = 0.1265`
> （2000 shots/setting、settings=1、Weyl 摂動評価）。
> **⚠️ この条件は未達である。** 物理的な（非負 substochastic な）構成では 10⁴反復・8リスタート・
> 台分割の掃引を尽くしても最良 0.0666（要求の **0.53倍**）、有理化後 0.0536（**0.42倍**）。
> 従来の GO 判定は、レンジ幅2の正規化に対しレンジ幅1の二項上界を当てていたため2倍緩かった
> （`p0-certificate-spec.md` §4quinquies.3）。
>
...
>
> **構造的な緊張（2026-07-26 に判明、本提案の核心的な弱点）:** 入れ子条件は
> `f_u · b_0 = 0` を要求し、これは隠れ状態の**1段での励起を禁じる**。隠れ状態は
> `b_0 → supp(f_u) → 隠れ状態` の2段でしか到達できず substochastic な減衰を2回受けるため、
> 第7方向の振幅は他の6方向より構造的に小さくなる（実測 `σ₁ ≈ 4.4` に対し `σ₂…σ₇ ≈ 0.05–0.11`）。
> **入れ子条件 (i) と有限shot余裕 (iii) は互いに反発する。** 最適化の失敗ではない。
>
> **帰結（資源下界）:** D_cl ≤ 6 のいかなる非負古典 HMM も 𝒲_full 上のデータを再現できない
> （ノイズなしの理想データに対しては (ii) により**証明済み**。有限shotでの認証は (iii) が未達のため未成立）。
...
- **数値的土台（実施済み）:** ステップ2スモーク（`scripts/sigma7_smoke.py`）— Hankel 43×43、H_cal 7×7、opnorm 正規化採用、`σ₇(H_full)` は射影付き山登りで 0.609 到達（要求 0.2933）、判定 GO。
  続くステップ2bis（`scripts/minor7_smoke.py`）で**認証対象を 8×8 部分行列へ差し替え**。
  **⚠️ ただしステップ2・2bis の GO 判定は閾値の正規化誤りにより2倍緩かった**（§6 参照）。
  訂正後はステップ2が 1.04×、ステップ2bis は 0.76× で **FAIL**。
- **ステップ3–4（実施済み、2026-07-26）:** `scripts/step3_construct.py` / `step4_certify.py`。
  全体を7状態 controlled sub-Markov 過程として作ることで、calibration 一致が**恒等式として**成立し
  `rank(H_cal) ≤ 6` が自動になった（根探索が不要）。exact 認証 C1–C5 すべて PASS。
  一方 `σ₇(H_wit)` は要求の 0.42–0.53倍で未達。
...
**定義 1（観測インターフェース）.** alphabet サイズ m、prefix/suffix 長 ≤2 の語集合を固定する。実測するのは全 1555 語ではなく、**calibration の 43 語（長さ ≤2）と witness の 64 語（`H_wit` を張る深さ4語）の計 107 語**である。
**各値 `R(w)` は条件付き確率 `∈ [0,1]`**（語 w が protocol、n shots で二項推定）。
`R(w) ∈ [0,1]` は**すべての語**で成り立たねばならない — 測っていない語で負値を取る構成は物理的でなく、実際に一度そのような構成を作って破棄した（`p0-certificate-spec.md` §4quinquies）。
これを保証する十分条件として、**全体を非負 substochastic な過程に取る**（`V_u ≥ 0`、列和 ≤1）。
> ⚠️ **旧記述の訂正:** 当初は opnorm 正規化 `‖V_u‖₂ ≤ 1`（`|R(w)| ≤ 1`、レンジ幅2）を採りつつ、
> 雑音には `[0,1]` の割合に対する二項上界 `1/(2√n)` を当てていた。両者は整合しない。
> **スケール不変な要求は `σ₇/（レンジ幅） ≥ 2√N/√n`** である。

shot 予算は凍結値（calibration ≤ 1.2×10⁵ shots、held-out 独立予算 ≤ 1.28×10⁵、1000–2000 shots/setting、settings ≤ 2、標準は 1）。

**定義 2（凍結資源クラス 𝔇₆）.** 状態数 `D_cl ≤ 6`、非負遷移 `T^{cl}_u ∈ ℝ_{≥0}^{6×6}`（列和 ≤1 の substochastic）、非負初期分布 `b_0`（`𝟙† b_0 = 1`）を持つ古典 controlled sub-Markov 模型の全体。正規化規約はステップ3で凍結済み（`certificates/p0d_certificate_2026-07-26.txt`）。
...
**主命題（= §0 の boxed 命題）.** P0-D 証明書を持つ組が存在する。
**status（2026-07-26）: (i)(ii) は Exact（明示構成で証明済み）、(iii) は凍結予算で未達。**
明示構成は `certificates/p0d_certificate_2026-07-26.txt`（有理数データ＋C1–C5 の判定）。
...
2. **ℱ_full のブロック接木 ansatz**（**2026-07-26 修正**。当初の記述は条件が不足していた）**:**
   ```
   V_u = [ T^{cl}_u   x_u ]      c = [ b_0 ]      p = [ 𝟙 ]
         [ f_u†       g_u ]          [ 0  ]          [ ρ_u… 実際は p_7 ≠ 0 ]
   ```
   **正しい十分条件は `p_7 = 0` かつ `f_u† b_0 = 0`（∀u）の2つ。** 当初は `f_u† b_0 = 0` と `c_7 = 0`
   のみを挙げていたが、`p_7 ≠ 0` だと長さ2で寄与が出てしまうため不足だった（SymPy で検算済み）。
   この2条件の下で 𝒲_cal 上の `P_{M_D}(w) = R_full(w)` が**恒等的に**成立し、逸脱は長さ3から
   `(𝟙† x_t)·(f_v† T^{cl}_u b_0)` を通じて現れる。さらに **`rank(H_cal) ≤ 6` も自動成立**する
   （`|w| ≤ 1` の到達ベクトルが `{x_7 = 0}` に載るため）。
   **全体を非負 substochastic に取る**（`V_u ≥ 0`、列和 ≤1）と `R(w) ∈ [0,1]` が全語で保証され、
   非負の世界では `f_u† b_0 = 0` は台の disjoint 性 `supp(f_u) ∩ supp(b_0) = ∅` と同値になる。
3. **正規化:** `‖V_u‖₂ ≤ 1` を射影（特異値クリップ後に非負ブロック構造へ再射影）で維持。opnorm 規約はステップ2の凍結決定。
4. **最適化（実施済み・未達）:** 目的関数 `σ₇(H_wit)`（8×8 の選択は最大体積ヒューリスティクス）、
   制約付き射影山登り。10⁴反復・8リスタート・`|supp(b_0)| = 2,3,4,5` の掃引で最良 **0.0666**
   （要求 0.1265 の **0.53倍**）。これは §6 の stop condition (1) に抵触する。
   原因は §0 に記した**構造的緊張**であり、反復を延ばしても埋まらないと判断する。
5. **有理化（実施済み）:** 分母 `10⁵` へ丸め、非負性・列和・台条件を `Q` 上で厳密に再充足。
   `σ₇(H_wit)` は 0.0666 → 0.0536 へ約2割劣化した。
...
- **最も弱い仮定 → 2026-07-26 に折れた:** 「接木 ansatz が非負制約と同時に `σ₇(H_wit) ≥ 0.1265` を許す」
  という仮定は**成立しなかった**。実測で要求の 0.42–0.53倍。原因は §0 に記した構造的緊張
  （入れ子条件が隠れ状態の1段励起を禁じ、第7方向が2段の substochastic 減衰を受ける）。
  **中心命題の (iii) はこの一点で止まっている。**
- **最も脆い数値操作:** (a) 有理化での `σ₇(H_wit)` 劣化（実測で約2割：0.0666 → 0.0536）、
  (b) minor 選択の pivot 依存、(c) **`τ_H` の評価式そのもの** — `(√a+√b)σ_entry` は独立同分布成分を
  仮定した乱雑行列の**漸近**評価であって確率1の上界ではなく、8×8 での妥当性は未検証（ステップ5）。
  (d) **そもそも作用素ノルム＋Weyl は「rank ≥ 7 を結論する十分条件」にすぎず、最悪ケース評価として
  極端に保守的**である。必要なのは「凍結資源クラス 𝔇₆ の最良元がデータに適合しない」ことの検定であり、
  64測定を統計量へ集約する尤度比検定の方が shot 効率は遥かに良いはずである。**ここが最大の改善余地。**
...
- **stop condition:** (1) ステップ A の最適化が 10⁴ 反復オーダーで `σ₇(H_wit) ≥ 0.1265` に到達しない、(2) kill test D が成立、(3) ステップ C3 で非負性が空転と判明、(4) Stage 2 の 3 チャット判定で 1 回でも還元成功 — のいずれかで中心主張を撤回・縮小し §6.4 へ記録する。
  ...
  > ⚠️ **(1) は 2026-07-26 に発火した**（10⁴反復・8リスタート・台分割掃引で最良 0.53×）。
  > ただし**撤回ではなく縮小・再設計**とする。理由：(i)(ii) は証明できており、
  > 失敗しているのは**判定方式**（作用素ノルム＋Weyl という保守的な十分条件）であって
  > 現象そのものではないため。選択肢は `p0-certificate-spec.md` §4quinquies.4 の (a)–(d)。
  > **(c)（統計的検定への置換）を試して、なお届かなければ撤回する。**
