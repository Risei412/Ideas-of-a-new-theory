...
This script is intentionally platform-minimal.  It tests the load-bearing
constraint that a stochastic first-success charge trajectory imprints a known
nuclear phase when the nuclear Hamiltonian depends on charge state.

Non-claim: repeat-until-success control and measurement feedback are not new.
The candidate residue is the NV-specific hardware completion cost once charge
readout, dwell-time resolution, and charge-dependent hyperfine dynamics are
fixed.
"""
...
def discarded_transcript_coherence(p: float, phi: float) -> complex:
    """Geometric first-success average sum_n p(1-p)^n exp(-i n phi)."""
    if not (0.0 < p <= 1.0):
...
def main() -> None:
    ...
    # Core deletion: no charge-dependent phase mismatch.
    ...
    # Illustrative timing requirements for a differential 80 Hz precession.
    dw = 2 * math.pi * 80
