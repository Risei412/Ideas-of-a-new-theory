...
**S1 = GKSL pullback 完了【2026-07-26 同日】— 具体的準位系で実現。**
...
- **tripod 型5準位**（`\|1⟩` 参照／`\|2⟩,\|3⟩` 準安定＝保護（k=2）／`\|4⟩,\|5⟩` 励起＝高速）。
  全レート非負、`H=H†`、`ρ⁽⁰⁾=\|1⟩⟨1\|` が厳密に定常、源は EIT 規約の `c = 𝒱_p ρ⁽⁰⁾`。
  **源・読み出しともに P 支持**（Z8 の制約を正面から満たす）。
- **縮約4×4ペンシルが完全 25×25 Liouvillian と厳密な恒等式として一致。**
  独立 float 実装との差 `7.24×10⁻¹⁷`（`prop:phase-h` の `4.5×10⁻¹⁶` より小さい）。
- **両汎関数で符号条件が成立** ⇒ 分散稜線 `Γ²ε = 25527228/1405495 = 18.162`、
  吸収稜線 `Γ²ε = 3677595/2243752 = 1.639`、**比 11.08 倍**、`Δ_split = 16.523`。
  全次数 Sturm 認証で有限 `(Γ,ε)` の実根として確認、2-jet への収束は厳密に `O(Γ⁻¹)`。
- **🆕 凍結理論の空白を2つ埋めた。** (i) **純位相緩和の縮約則
  `−½γ_φ(a_j−a_ref)²` を初めて導出**（EIT L.1115 が明示的に先送り、SMRT には "dephas" が皆無。
  LKCT の中心 `(D_L)_PP` は一度も jump から導出されていなかった）。副産物として
  **`D_L` は対角に強制される**という構造的制約が判明。(ii) **弱プローブ線形化そのもの** —
  SMRT `prop:phase-n` は「linearization gives exactly the pencil」と主張するだけで `½` すら
  計算していない（`rem:gksl-model` は導出を要求）。導出し、凍結の主張と一致を確認。
- **🆕 低速減衰は分裂の必要条件（V6）。** `γ_j → 0` にすると `B_PP` が純虚になり
  `κ, α` が整列して `Im(κ conj α)=0`、**分裂が消える**。`prop:phase-n` の `B_jj = 0` は使えない。
- **🆕 2つの稜線は漸近領域に入る `Γ` が桁で異なる**（吸収 ~2×10²、分散 ~2×10³）。
  両稜線の同時観測には `Γ ≳ 2×10³` が要る。新たな実験上の制約。
- **NC5（jump 向き反転）が有益:** 保護ブロックの存在は jump の**向き**に依存する
  （`\|1⟩⟨j\|` 配置でのみ準安定準位が保護される）。

**S1 実験可視性の判定 完了【2026-07-26 同日】— 実現可能域は有界な窓。**
`docs/lkct-visibility-audit.md`、`scripts/lkct_visibility.py`（seed 20260732、U0–U8 全 PASS）、
`certificates/lkct_visibility_2026-07-26.txt`。**論理 status は Conditional / Model-specific**
（ガイド §10.3 カテゴリ D。Exact ではない）。

- **物質固有の数値を一切使わない判定**にした。リポジトリに物質パラメータは存在せず
  （単位つき数値は全体で `139 MHz` の1件のみ、それもモデル出力）、egress では
  文献監査仕様 §4.2 が要求する DOI 2経路検証ができないため。判定は2つの無次元比のみに依存する:
  `R_Γ = Γ_opt/γ_int`（光学 damping／保護コヒーレンス固有減衰）と `η = γ_deph/γ_int`。
- **実現可能域 `8.2×10³ ≲ R_Γ ≲ 2.5×10⁴`（幅およそ3倍）。窓の内側で2稜線は 11.8–19.3 倍離れる。**
- **🆕 下限＝分散稜線の生成閾値。** `R_Γ,c ∈ (8148, 8203]` 未満では分散稜線が**存在しない**
  （吸収稜線のみ）。**2-jet はこの閾値を予測できない** — 前回の「`Γ ≳ 2×10³`」は
  2-jet が当たる領域であって稜線の存在領域ではなかった。判定はすべて厳密応答から行った。
- **🆕 上限＝信号の床。** 傾き `S ∝ 1/Γ` で減衰するため `R_Γ` を大きくしすぎると見えなくなる。
  `R_Γ,max ∈ [2.52×10⁴, 2.58×10⁴)`。**前回監査までに出ていなかった制約。**
- **🆕 EIT/ATS 判別子を3つ確立**（§10.3 D-4）。最も強いのは control power の**冪の違い**:
  稜線位置は `λ²`、ATS の分裂は `λ`。ほかに ground coherence 依存（dephaser を外すと消える）、
  Γ スケーリング指数（稜線 −1.20 vs 第2根 +0.20、指数差 1.40）。
- **🆕 単位変換の自動テストを新規作成**（no-go 12 が要求。リポジトリに機構が存在しなかった）。
  `(R_Γ, η) → (Γ, ε) → (R_Γ, η)` の往復が厳密に恒等。EIT の `Γ_XY/4` が本モデルへ
  **転写されない**ことも明示検査（係数が 4 と 2 で異なる）。
- **📌 判定基準の訂正を2件、正直に記録した。** U5 の「全点で床超え」は物理的発見を
  失敗としか記録できない表現だったため「窓が空でないか」へ改めた（閾値自体は不変）。
  U6(c) の「`η·Γ` が10%以内で一定」は設計ミス（漸近でしか成り立たない）で、
  スケーリング**指数差**による正しい基準へ訂正した。**当初基準の失敗も証明書に残してある。**
- **⚠️ ガイド §4.1 の事実誤認を訂正した。** 同表の「Rb/NV/SiV/SnV のsanity-check分類」は誤り
  （EIT 凍結文書に Rb・SiV・SnV は0件、NV は6件すべて定性的）。

**S1 検出模型の独立導出 完了【2026-07-26 同日】— §1.2 条件6 も条件付きで解消。**
`docs/lkct-detection-audit.md`、`scripts/lkct_detection.py`（seed 20260733、E0–E5 全 PASS）、
`certificates/lkct_detection_2026-07-26.txt`。status は Conditional / Model-specific。

- **継承ではなく導出。** 入力は本リポジトリで厳密確立済みの2量のみ —
  稜線の規格化傾き `S²`（厳密有理数）と control-power 則 `ridge ∝ λ²`（U6a）。
- **[D2] ショット予算:** `N_min = z*²/(Δ²·S²)` は厳密有理数。最適動作点は
  **`Γ=1500`（`R_Γ ≈ 1.05×10⁴`）で `N* ≈ 6.08×10⁶` shots/点**。10⁶ では窓が**空**、
  10⁷ で開き、10⁸ で全格子。**実用窓は存在窓より両側とも狭い** —
  分散枝の傾きは生成閾値で 0 から立ち上がる（接根）ため。
- **U5 の `FLOOR = 10⁻³` の物理的正当化:** `N = 3.6×10⁷` shots/点という**資源宣言と等価**。
- **[D3] オフセット:** 許容残差 `b_max,rel = S·Δθ_max ≈ 5×10⁻⁴`。分裂の同定は頑健。
- **[D4] 伝播:** λ² 則＋Beer–Lambert から **`OD ≤ −2ln(1−δ_I) ≈ 0.103`** を導出。
  3点分布の厳密平均応答で交差が生存（シフト +0.0016〜0.0021、予測 1/600 と同桁）。
- **⇒ §1.2 降格条件は 2・6 とも（検出模型に条件付きで）外れた。投稿先評価は PRA。**
  残余: 集団効果・検出器実装・高次スペクトル拡散（範囲外と明記）。

**次工程:** (0) **赤チーム Stage 1–2 と PRA 原稿 Stage 0 起草**（S1 の監査は一巡した）、
(2) 符号条件を満たすパラメータ領域の分類、
(3) `k≥3` と一般 `k` の定理、(4) path-coherent な低速 jump による L2 回避経路の列挙、
(5) 符号付き／実トロピカル幾何の文献監査（**未実施・egress 必要**）、(6) A1/A3/A4 判別と赤チーム Stage 1–2。
