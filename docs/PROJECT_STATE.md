...
**判定完了【2026-07-26、作成同日】— `docs/dark-ridge-novelty-verdict.md`。三準位で FREEZE。**
`scripts/dark_ridge_lambda_model.py`（T0–T4・P1–P3 全PASS、exact arithmetic、
証明書 `certificates/dark_ridge_three_level_2026-07-26.txt`）で次を厳密に示した。
- **P1: `Ω_p=0` の定常状態は厳密に `|g₁⟩⟨g₁|`**（全パラメータ）。control は `g₂–e` のみを結び
  `g₁` は流出のない sink になるため、**optical pumping による population inversion は起きない。**
  分岐比 `Γ₁/Γ_e` は `ρ⁰` に影響しない。事前に想定した最大のリスクが消えた。
- **P2: 吸収回復曲線は存在しない。** `Im χ` の分子 `γ_31(γ_12²+δ²)+βγ_12` は
  全単項式係数が正で、`γ_12>0` なら厳密に正。
  **⇒ EIT 弱プローブという具体的 GKSL クラスでは受動性が符号を強制する**
  （LKCT 監査 §6 Y7 の「CPTP は符号を強制しない」は線形代数レベルの話であり、
  同 §6 の留保「Step 2 で導出するまで no-go 分岐を捨ててはならない」が正しかった）。
- **P3: 分散回復曲線は存在するが `β = Δ_p(γ_12²+δ²)/δ`** で、`γ_12→0` で `β = Δ_p·δ`
  ＝ **ac-Stark シフトによる既知の分散零点へ還元**。計算計画 §21 停止条件3 が発火。
- 付随して `γ_31 = Γ_e/2 + γ_12/4`（naive な `Γ_e/2` ではない）を確認。
- 36点の exact スキャンでも `D only` 9 / `none` 27、**`A only`・`D+A` は0点**。

⇒ クラス **D only**、`Δ_split` は片側が空で定義されない。固有性レベルは **L1**（L2 未満）。
**三準位での「吸収・分散回復点分裂」は死んだ。** ただし提案24 全体の却下ではない
（死因は物理 pullback 側で、抽象応答族の数学は生きている）。§6.4 への記録は提案24 の最終判定まで保留。
残る経路は「受動性が吸収回復曲線を禁じる最小条件は何か」という **no-go 分類（PRL 分岐）**への組み替え。

**Gate P1 完了【2026-07-26、同日】— その no-go も反例で死亡。路線全体を FREEZE。**
`scripts/dark_ridge_pump_probe.py`（規約テスト全PASS、exact、証明書
`certificates/dark_ridge_nogo_2026-07-26.txt`）。`g₁` の sink 性だけを壊す jump を**1本だけ**追加
（`L_w=√w|g₂⟩⟨g₁|` または `L_r=√r|e⟩⟨g₁|`、ガイド §4.4 の予算内）すると、
**`ρ_ee−ρ_{g₁g₁}≈−0.55`（population inversion なし）のまま `Im χ` が符号を変え、利得へ抜ける。**
`rate→0` で三準位の結果を厳密に回収するので、追加機構が効いていることも確認済み。
⇒ **命題「dark state があれば `Im χ` は符号を変えない」は偽。** そして反例は
**反転なし利得（LWI）**であり、確立した分野に入るだけである。
§21 停止条件3（分散側）と、機構が dark-state 固有でない事態（吸収側）が同時に発火した。
**残す資産は3点** — 三準位の厳密感受率、符号を破る負制御、そして機構的必要条件
「`Im χ` の符号を固定するのは dark-state 構造ではなく `ρ⁰` の population 配置」。
却下記録はガイド §6.4・`docs/context-pack.md` §5 に同一コミットで転記済み。
**提案24 の番号は消費しない**（死んだのは物理 pullback で、抽象応答族の数学は生きている）。
