...
> **【2026-07-25 方針転換】P0-D は完成したが、固有現象への経路ではないと判明した。**
> 導出可能性の監査 → **`docs/p0d-derivability-audit.md`**（本節の下段に要約）。
> **rank証明書を積み増す路線（下記ステップ5–7）は凍結。** 次に叩くのは CIRT の受動性障害。

**次の一手（優先度順）:**

1. **CIRT 定理C4 の一般証明に着手する** — スモークは通った（下記）が未証明。
   設計図に「C4が偽なら理論は終わる」と明記された要石。Herglotz類が Schur補元と
   受動的相互接続で閉じることを、有限受動鎖に限らない形で示す
2. **CIRT の CG1（exact seed）** — §4.4 の `M=[[1,2],[2,1]]` 反例を
   実際の有限次元GKSL族（qutritまたはΛ系、2ポート以上）に埋め込み、**違反領域が有限幅**
   であることを示す（ガイド §6.5 の「孤立した厳密零は不十分」を踏まないため）
3. **P0-D の余次元を数える** — RISEI `ass:robustness` が要求する
   "clearly identified finite codimension"。calibration一致を「測定誤差内」の開条件に
   言い換える作業と併せて実施（`p0d-derivability-audit.md` §5）
4. DCIT を続けるなら **Priority 4（response pullback）が先** — 現状 DCIT は
   時間応答への写像を持たないため、そもそも `R_*` を導出できる状態にない

**凍結した路線:** ~~5. `τ_H(α)` 導出~~／~~6. P0-A・P0-E の明示構成~~／~~7. 投稿先判定~~ —
族Aの予算（qutrit で 324）に対し rank 証明書は桁が足りず、かつ **rank障害は自由度追加で消える**
（下記）。証明書を積み増して P0 全体に到達する見込みがないため中断する。

---

**導出可能性の監査（`docs/p0d-derivability-audit.md`）— 結論: 導出は可能、しかし固有性は出ない:**
- **RISEI は形を供給できる（肯定的結果・資産）** — `def:protocol` は順序付きリスト、
  propagator は時間順序積。窓を互いに素・固定長にすれば `U_π = M_{w_k}⋯M_{w_1}`、
  `M_u = exp(L_u Δt)` は CPTP。POVM読み出しで `R(w)=Tr[E M_w ρ₀] ∈ [0,1]` が自動で従う
- **しかし桁が足りない** — `scripts/p0d_family_a_audit.py`（厳密有理数）で **`dim K = 7`** が確定。
  rank 7 を張るには最小 qutrit（`d²≥7`）で、そこでの族A予算 `(3·6)²=324` は **46倍**。
  **P0-D証明書は族Aに対する証拠を一切与えない**（「還元された」ではなく「排除できない」）
- **既に7回失敗した形である** — CIRT §0 の失敗監査表に
  `resource-separation matched pair | FAIL` と `ternary irreducible response | FAIL | 隠れMarkov模型`
  が既載。診断は「**rank を含む組合せ論的障害は自由度を足せば必ず消える。7回連続で起きた**」。
  ガイド §6.5 の「局所同値 → 大域的obstruction → 有限資源certificate だけでは固有性にならない」も同旨
- **DCIT は現時点では導出すらできない** — witness `W_□=√2r−1` は静的なPSD completion量で、
  時間方向への写像（Priority 4）が未着手

**CIRT 定理C4 スモーク（`scripts/cirt_c4_smoke.py`）— 判定 GO（反例なし）:**
- 受動ancilla鎖 **2000例（深さ1–5、鎖形・スペクトル形の2系統）で違反 0件**
- 対照群は違反 1653件を検出（Im h 1581／C2a 809／L 1432）— **検定力は確認済み**
- **C2a を発火させられるのは不定符号スペクトル測度の対照のみ**（利得項の対照では0件）。
  その最小固有値は −4.95 に達し、CIRT §4.4 の標的（−1.0）を検出できる感度がある
- **反例の不在であって C4 の証明ではない。** 有限次元・有限深さの受動鎖に限った検査
