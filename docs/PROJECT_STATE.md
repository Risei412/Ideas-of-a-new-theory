...
**⇒ §1.2 条件2「experimental pullback ができず抽象 parameter のまま」は局所応答の水準で解消。
ただし条件6「full physical model では現象が観測不能」が部分的に残る**
（optical depth・アンサンブル伝播・検出器雑音は §4.3 により凍結からの継承が禁じられ範囲外）。
**したがって S1 は「投稿可能水準に到達した」とは言い切れない。投稿先は PRA のまま。**

**次工程:** (1) **床 `FLOOR` の物理的正当化**（窓の上限を確定させる）と
optical depth・伝播・検出器雑音の独立導出（§1.2 条件6 の完全解消）、
