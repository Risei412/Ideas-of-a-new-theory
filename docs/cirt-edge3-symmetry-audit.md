...
1. ~~**極をデルタ関数として扱っている。**~~ → **解消済み（2026-07-26、`docs/cirt-window-honesty-audit.md`）。**
   T7 が各極を HWHM $\Gamma_L$ の Lorentzian に置き換えて検査した結果、**判定は ALIVE**。
   有効な広がりは $\Gamma_L\in[10^{-4},1/2]$ の**4桁**にわたり、掃引範囲で偽陽性は一度も出なかった。
   点極理想化は結論を作っていなかった。CIRT §9.1 の第4停止条件は発火しない。
...
4. ~~**window honesty 検査を次の作業とする**~~ → **完了（2026-07-26、`docs/cirt-window-honesty-audit.md`）。
   判定 ALIVE。** 副産物として C10（定量Loewner）に定量的入力が入った——
