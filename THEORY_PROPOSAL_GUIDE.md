...
| **機能階層障害理論（FHOT／提案25、Φ階層排他ゼロ）** | **有限次元トモグラフィ完全性への還元。** 先行研究照合は不要（自己内部監査で確定） | **早期kill**（Stage 0 内部監査、作成同日） | **一般教訓:** 有限次元・トモグラフィ可能な設定で、状態の非線形汎関数（QFI等）は線形汎関数全体から決まるため「線形汎関数に盲目・非線形汎関数に可視」という枠組み自体が原理的に空。**次にNonlinear functional hierarchy（P1）を狙うなら、資源制限（P0-2）か無限次元のどちらかを同時に導入しない限り成立しない。** **残る資産（機構的必要条件）:** population の正値性 `\|D_A\|≤Σ_A` から、任意の scaling path で `ν(F_Q)<ν(χ) ⟺ ν(g)<ν(Σ_A)−ν(D_A)≤0`——QFIが線形応答を漸近的に追い越すには probe coherence の減衰自体が消えること（保護されたcoherence）が必要で、population再配分だけでは不可能。保護の源泉は非線形性ではなく線形理論側のkernel構造（凍結理論の `ker D≠0` へ差し戻し）。検定器 `scripts/fhot_smoke.py`（GKSL superoperator層・SLD/QFIの2独立実装）は再利用可 | 2026-07-26 |
